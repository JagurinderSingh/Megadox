...
def log_date_list(label, values, limit=25):
  if not values:
    log_kv(label, "None")
    return
  display_values = values[:limit]
  suffix = "" if len(values) <= limit else f" ... +{len(values) - limit} more"
  log_kv(label, f"{display_values}{suffix}")
...
def database_engine_connection():
  ...
  log_kv("DB target", "postgresql://postgres:***@localhost:5432/index_value_strategy")
...
if proceed == 1:
  ...
  log_date_list("NSE extra dates", sorted_extra_ingested_list_nse)
  log_date_list("BSE extra dates", sorted_extra_ingested_list_bse)
